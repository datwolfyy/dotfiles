...
-- options: white, forest, nge, edgy, cthulu
local ntheme = 3
local themes = { "white", "forest", "nge", "edgy", "cthulu" }
beautiful.init("/home/bendeguz/.config/awesome/themes/" .. themes[ntheme] .. "/theme.lua")
local __theme = beautiful.get()
--local nice = require("nice")
--comment out to disable
-- Notification separator widget
...
nice {
	titlebar_color = "#313a4a",
	titlebar_padding_left = 5,
	titlebar_padding_right = 5,
	titlebar_height = 30,
	titlebar_font = "Nunito Regular 11",
	win_shade_enabled = false,
	no_titlebar_maximized = true,
	floating_color = rb_color,
	ontop_color = rb_color,
	sticky_color = rb_color,
}
end
...
screen.connect_signal("request::desktop_decoration", function(s)
    ...
    --[[local layout_list = awful.widget.layoutlist {
	    base_layout = wibox.widget {
		    spacing		= 6,
		    forced_num_cols	= 1,
		    layout		= wibow.layout.grid.vertical,
	    },
	    widget_template = {
		    {
			    {
    ...
    awful.keygrabber {
	    ...
	    keybindings = {
		    ...
		    {{ modkey, 'Shift' }, ' ', function()
			    awful.layout.set(gears.table.cycle_value(layout.list.layouts, layout_list.current_layout, -1), nil)
		    end},
	    }
    }--]]
...
--[[    awful.key({ modkey, "Control" }, "=", function ()
	      ...
	      {description = "cycle theme forward by 1", group = "awesome"}),
    awful.key({ modkey, "Control" }, "-", function ()
	    					ntheme=ntheme-1
						if ntheme==0 then
							ntheme=table.getn(themes)
						end
						awesome.restart()
					  end,
	      {description = "cycle theme backwards by 1", group = "awesome"}),
--]]
    awful.key({ modkey }, "x",
              function ()
                  awful.prompt.run {
